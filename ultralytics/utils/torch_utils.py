...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    """model：需要分析的深度学习模型对象  detailed:用于指定是否显示详细的模型参数信息  verbose：用于指定是否打印模型信息  """
    if not verbose:
        return
    n_p = get_num_params(model)  # number of parameters 参数数量,这是指模型中可学习的权重和偏置的数量
    n_g = get_num_gradients(model)  # number of gradients   梯度数量
    n_l = len(list(model.modules()))  # number of layers    计算模型的层数，通过遍历模型的所有子模块并计数子模块的数量
    # 如果 verbose 参数为 True，则会打印模型的信息，包括模型的名称、层数、参数数量、梯度数量以及计算量（FLOPs
    # 如果 detailed 参数为 True，还会打印详细的模型参数信息，包括每一层的名称、是否需要梯度、参数数量、参数形状、参数均值、参数标准差和参数数据类型
    if detailed:
        LOGGER.info(
    ...
    # 假如verbose不为False，函数最后会返回一个包含模型信息的元组，包括层数、参数数量、梯度数量和计算量
...
# 打印的参数看LOGGER.info
# 用于对模型的速度、内存使用和计算量（FLOPs）进行性能分析。这个函数接受输入数据和操作（通常是模型的前向传播操作）
