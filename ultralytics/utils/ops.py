...
def soft_nms(bboxes, scores, iou_thresh=0.5, sigma=0.5, score_threshold=0.25):
    ...
    while order.numel() > 1:
        ...
        iou = bbox_iou_for_nms(bboxes[i:i + 1], bboxes[order[1:]], GIoU=False, DIoU=False, CIoU=False, EIoU=False,
                               SIoU=False, ShapeIoU=False, scale=0.0).squeeze()
...
#     #             keep.append(i)
#     #         # 设置IOU类型
#     #         iou = bbox_iou_for_nms(bboxes[i], bboxes[order[1:]], CIou=True).squeeze()
#     #
#     #         idx = (iou > iou_thresh).nonzero().squeeze()
#     #         if idx.numel() > 0:
#     #             iou = iou[idx]
#     #             nexScores = torch.exp(-torch.pow(iou, 2) / sigma)
#     #             scores[order[idx + 1]] *= newScores
#     #
#     #         newOrder = (scores[order[1:]] > score_threshold).nonzero().squeeze()
#     #         if newOrder.numel == 0:
